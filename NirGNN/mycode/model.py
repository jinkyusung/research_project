...
class SessionGraph(nn.Module):  # construct session graph
    ...
    def bhattacharyya_distance(self, vector1, vector2):
        # Jaccard distance between a point and a sample set
    ...
    def zeroshot(self, intent_item , intent_attribute, candidate_attribute,mask,alias_inputs):
        ...
        distanceb = self.bhattacharyya_distance(self.linear_zero(intent),oriitem)

        loss = torch.sum( torch.LongTensor(distanceb) )
...
def forward(model, i, data, attr_data, taxo_data):
    alias_inputs, A, items, mask, targets, attr1, attr2, taxo1, taxo2, taxo3, ca_a1, ca_a2 = data.get_slice(i, attr_data, taxo_data)
